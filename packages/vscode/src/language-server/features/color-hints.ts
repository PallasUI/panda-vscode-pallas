...
import { getRecipeColorHints } from './recipe-color-hints'
...
export function registerColorHints(lsp: PandaLanguageServer) {
  ...
  lsp.connection.onDocumentColor(
    tryCatch(async (params) => {
      ...
      // Initialize color information array
      const colors: ColorInformation[] = []

      // Get standard token color hints
      ...
      // Get recipe-based color hints
      const recipeColors = await getRecipeColorHints(lsp, doc)
      colors.push(...recipeColors)
